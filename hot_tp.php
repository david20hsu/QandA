...
if (isset($_POST["editsubmit"])) {
    ...
    $cql = "UPDATE `hot_tp` SET `tp_name`=:tp_name,`username`=:username where `tp_id`=:tp_id";
...
if (isset($_POST["addsubmit"])) {
    ...
    $cql = "INSERT INTO `hot_tp` (`tp_name`,`username`) Values(:tp_name,:username)";
    $stmt = $conn->prepare($cql);
    $stmt->execute($data);
    //     $conn = null;
}
...
<div class="container-fluid">
    ...
    <table id="myDataTalbe" class="table table-bordered table-striped table-hove" style="width:90%">
        ...
        <tbody>
            ...
            $sql = "select *  from hot_tp order by tp_id";
            ...
            ?>
                <tr>
                    ...
                    <td>
                        <a href="#updModal<?php echo $row['tp_id']; ?>" data-toggle="modal" class="btn-sm btn-info mt-1">編修</a>
                        <a onclick="return confirm('確定要刪除嗎 ?')" href="cat/hot_tpdel.php?del=<?php echo $row['tp_id']; ?>" class='btn-sm btn-danger mt-1'>刪除</a>
                    </td>
                </tr>
                <div class="modal fade" id="updModal<?php echo $row['tp_id'] ?>">
                        ...
                        <div class="modal-content">
                            ...
                            <form class="form-horizontal" action="#" method="POST" name="edit_Form" id="edit_Form">
                                <div class="modal-body">
                                    ...
                                    <input type="hidden" name="tp_id" id="tp_id" value="<?php echo $row['tp_id'] ?>">
                                    <div class="form-group">
                                        <label for="tp_name" class="col-sm-5 control-label">類別名稱<span class="required">*</span>:</label>
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="tp_namex" name="tp_namex" value="<?php echo $row['tp_name'] ?>" required placeholder="時段名稱">
                                        </div>
                                    </div>
                                </div>
                ...
                <form class="form-horizontal" method="POST" id="insert_form">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="tp_name" class="col-sm-5 control-label">類別名稱<span class="required">*</span>:</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control" id="tp_name" name="tp_name" required placeholder="時段名稱">
                            </div>
                        </div>
                    </div>
                    <!-- Modal 底部 -->
