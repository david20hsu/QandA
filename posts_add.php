...
if (isset($_POST["add_submit"])) {
  ...
  $cql = "INSERT INTO `qna_posts` (`title`,`message`,`cat_id`,`status`,`username`) VALUES(:title,:message,:cat_id,:status,:username)";
  $sth = $conn->prepare($cql);
  $sth->execute($data);
...
<form method="post" action='#'>
  ...
  <div class="row">
    ...
    <div class="col-4">
      <?php if ($id == "") { ?>
        <input type="submit" class="btn-sm btn-success mt-1" name="add_submit" value="新增">
      <?php } else { ?>
        <input type="submit" class="btn-sm btn-success mt-1" name="edit_submit" value="編修">
      <?php } ?>
